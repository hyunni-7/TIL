...
def respond_with_articles(request):
    ...
    if request.method == 'GET':
        ...
        serializer = ArticleSerializer(articles, many=True)  # serializing on "조회 데이터"
        return Response(serializer.data)  # 데이터 반환

    # POST --> 게시글 생성
    elif request.method == 'POST':
        serializer = ArticleSerializer(data=request.data)  # serializing on "request로 받은 데이터"
...
def respond_with_article(request, art_pk):
    ...
    # GET --> 특정 게시글 조회
    if request.method == 'GET':
        serializer = ArticleSerializer(article)  # serializing on "조회 데이터"
        return Response(serializer.data)  # 데이터 반환
